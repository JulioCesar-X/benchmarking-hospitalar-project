...
`;
    ...
    this.filterSubject
      .pipe(
        ...
        switchMap((filter) => {
          ...
          return this.indicatorService.getAllData({
            ...this.filter,
            ...filter,
          });
        })
  ...
  loadIndicatorName(): void {
    if (this.filter.indicatorId !== undefined) {
      this.indicatorService
        .showIndicator(Number(this.filter.indicatorId))
        .subscribe({
          next: (name) => {
            this.indicatorName = name.indicator_name;
            const service = name.sais?.find(
              (sai: any) => sai.service?.id === this.filter.serviceId
            )?.service;
            const activity = name.sais?.find(
              (sai: any) => sai.activity?.id === this.filter.activityId
            )?.activity;
    ...
    if (element) {
      ...
      html2canvas(element, { scale: 2 })
        .then((canvas) => {
          ...
          pdf.setFontSize(11);
          pdf.text(
            'Benchmarking Hospitais - Desempenho Assistencial',
            pdfWidth / 2,
            currentPdfHeight,
            { align: 'center' }
          );
          ...
          pdf.setFontSize(9);
          pdf.text(
            `Serviço: ${this.serviceName || 'N/A'}`,
            pageMargin,
            currentPdfHeight
          );
          currentPdfHeight += 10;
          pdf.text(
            `Atividade: ${this.activityName || 'N/A'}`,
            pageMargin,
            currentPdfHeight
          );
          currentPdfHeight += 10;
          pdf.text(
            `Indicador: ${this.indicatorName || 'N/A'}`,
            pageMargin,
            currentPdfHeight
          );
          currentPdfHeight += 10;
          pdf.text(
            `Tempo: ${this.filter.year}${(this.filter.month ?? 0)
              .toString()
              .padStart(2, '0')}`,
            pageMargin,
            currentPdfHeight
          ...
          const srcHeight = Math.min(
            remainingHeight,
            (pdfRemainingHeight * canvasWidth) / pdfWidth
          );
          pdf.addImage(
            imgData,
            'PNG',
            ...
            currentPdfHeight,
            pdfWidth,
            (srcHeight * pdfWidth) / canvasWidth,
            undefined,
            'SLOW',
            0
          );
  ...
  exportToExcel(): void {
    ...
    worksheet.getCell('A1').value = `Dados publicados a ${this.filter.year}${(
      this.filter.month ?? 0
    ).toString()}`;
    ...
    addDataToWorksheet(
      this.graphData.recordsAnualLastYear,
      'Produção Acumulada (Ano Anterior)',
      this.filter.year - 1,
      departamento
    );
