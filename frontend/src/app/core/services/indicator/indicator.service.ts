...
export class IndicatorService {
  ...
  getIndicatorsRecords(serviceId:number, activityId:number, date: string,pageIndex:number,pageSize:number): Observable<any> {
    return this.http.get<any>(`/indicators/sai/records?page=${pageIndex}&size=${pageSize}`).pipe(
      catchError(error => {
        console.error('Error fetching data:', error);
  ...
  getIndicatorsGoals(serviceId:number, activityId:number, year: number,pageIndex:number,pageSize:number): Observable<any>{
    return this.http.get<any>(`/indicators/sai/goals?page=${pageIndex}&size=${pageSize}`).pipe(
      catchError(error => {
        console.error('Error fetching data:', error);
        return throwError(() => new Error('Falha ao buscar indicadores c / metas'));
      })
