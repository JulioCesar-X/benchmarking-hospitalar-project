...
export class RecordsListSectionComponent
  implements OnInit, OnChanges, AfterViewInit
{
  ...
  loadRecords(pageIndex = 0, pageSize = 30): void {
    if (this.filter?.year && this.filter?.month && this.filter?.serviceId) {
      ...
      const activityId =
        this.filter.activityId !== null && this.filter.activityId !== undefined
          ? Number(this.filter.activityId)
          : null;

      this.indicatorService
        .getIndicatorsRecords(
          serviceId,
        ...
        .subscribe((response) => {
          const newRecords = response.data
            .map((item: any) => this.mapRecord(item, year, month))
            .flat();
          this.allRecords =
            pageIndex === 0 ? newRecords : [...this.allRecords, ...newRecords];
  ...
  onRecordsImported(records: any[]): void {
    const newRecords: Record[] = records.map((record) => {
      ...
      const existingRecord = this.allRecords.find(
        (r) => r.sai_id === record.sai_id && r.date === record.date
      );
    ...
    forkJoin(requests).subscribe(
      () => {
        this.setNotification(
          'Registros criados/atualizados com sucesso',
          'success'
        );
  ...
  exportRecords(): void {
    ...
    worksheet.getCell(
      'A1'
    ).value = `Dados publicados em ${this.filter.year}-${this.filter.month}`;
    worksheet.getCell('A1').font = { bold: true };

    const serviceName =
      this.allRecords.length > 0 ? this.allRecords[0].service_name : 'N/A';
    const activityName =
      this.allRecords.length > 0 ? this.allRecords[0].activity_name : 'N/A';
    ...
    const formattedTime = currentDate.toLocaleTimeString('pt-PT', {
      hour: '2-digit',
      minute: '2-digit',
      second: '2-digit',
    });
  ...
  editRecord(record: Record): void {
    ...
    if (
      (record.record_id !== null && record.record_id > 0) ||
      record.record_id === 0
    ) {
      record.isUpdating = true;
      this.recordService
        .updateRecord(record.record_id, record)
        .pipe(
          finalize(() => {
            record.isUpdating = false;
            record.isEditing = false;
    ...
    this.recordService
      ...
      .subscribe(
        () => {
          ...
          const recordToEdit = this.records.find(
            (record) => record.record_id === localRecord.record_id
          );
          if (recordToEdit) {
